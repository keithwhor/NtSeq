var Nt = function() {
  ...
  MatchMap.prototype.top = function(n) {
    ...
    return this.__orderedResults.slice(0, n).map(function(v) {
      return new MatchResult(this, v.pos, v.matches);
    });
  ...
  MatchMap.prototype.matchCount = function() {

    if (this.__matchCount) {
      return this.__matchCount;
    }

    var ordered = this.__orderedResults;
    var matchCount = makeArray(this.__query.size() + 1);
    ...
    for (var i = 0, len = ordered.length; i < len; i++) {
      ...
      if (num < maxMatch) {
        matchCount[maxMatch] = i - lastIndex;
        lastIndex = i;
        maxMatch = num;
      }
      if (num === 0) {
    ...
    return (this.__matchCount = matchCount);
