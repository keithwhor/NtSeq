var Nt = function() {
  ...
  function MatchMap(query, searchSpace, offset) {
    ...
    this.__initialize(offset);

  };

  MatchMap.prototype.__initialize = function(offset) {

    if (this.__initialized) {
      throw new Error('MatchMap has already been initialized');
    }

    offset = offset === undefined ? null : Math.max(0, offset | 0);
    this.__offset = offset;

    var noffset = offset | 0;
  ...
  MatchMap.prototype.sort = function() {
    ...
    this.__orderedResults = this.__results.slice().sort(function(a, b) { return b.matches - a.matches; });
